...
- **Decode seam (D6, locked):** **backend-edge decode.** `/ws/satellite` accepts binary messages (1-byte type prefix + session tag → demux); the backend decodes Opus → PCM at ingress into the *existing* chunk buffer. Everything downstream — STT call, speaker resolver, A1 segment, enrollment — remains PCM and untouched; the voice-server API and image are **frozen** (its `POST /api/voice/stt` keeps taking WAV). Cost: `opuslib`/libopus in the backend image + negligible CPU (mono 16 kHz, a handful of streams).
...
| **C1 (PR 1, with P0)** | Binary WS frames + Opus, backend-edge decode, capability-negotiated. Resolves the TODOS.md Opus P2. | `SATELLITE_OPUS_ENABLED` | — |
...
| D6 | Opus decodes at the backend edge; voice-server API frozen |
