...
- **#12 KB maintenance — classify unindexable vs transient 0-chunk docs** (also P2 below). Backend, testable on `.159`. Next up.
- **#10 Presence / media-follow room-switch latency (~1–2 min)** — adaptive hysteresis (also P2 below). Backend.
- **#6 Provisioning config drift** — commit accurate host_vars for all sats; inventory → mDNS + DHCP reservations. Ops (gitignored, no PR).
...
- ⬜ **Phase 1–2:** voice-server ONNX enroll endpoint + multi-sample cohesion-gated enroll service + `users`-link + guided frontend flow; purge the 41 polluted profiles + re-enroll the 3 members.
- ⬜ **Phase 3:** recalibrate threshold/margin from real enrolled separation; flip auto-enroll OFF (identify-vs-enrolled-only) + wire the review bucket.
